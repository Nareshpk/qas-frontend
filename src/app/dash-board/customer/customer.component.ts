...
export class CustomerComponent implements OnInit {
  ...
  onPdfButtonClick(params: any) {
    this.billManager.pdfId(params.data.slNo).subscribe((response) => {
      let date = new Date();
  ...
  onGeneratePdfReport(event: any, dateForm: any) {
    ...
    if (dayDate && this.dayilybutton) {
      this.dayvalue = 'Day';
      this.billManager.InvoicepdfId(date, this.dayvalue).subscribe((response) => {
        let date = new Date();
        let newDate = this.datevalue;
        saveAs(response, newDate);
      })
    }
